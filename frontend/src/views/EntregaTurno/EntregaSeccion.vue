    ...
    if (validate(rut)) {
        ...
        try {
            ...
            if (data != null) {
                ...
                props.form.errors[`entregados.${index}.run`] = "El run ingresado no existe en nuestros registros.";
            }
        } catch (error) {
            console.error({ error });
        }
...
if (props.updating) {
    onMounted(() => {
        ...
        props.form.entregados.forEach((entregado, index) => {
            props.formatearRut(entregado.run, props.form.entregados, index, 'entregados');
...
<template>
    <div class="bg-blue-200 mt-8 p-4 overflow-hidden shadow-xl sm:rounded-lg">
        <Label class="text-xl flex justify-center underline mb-4" for="tituloEntrega" value="Pacientes a entregar" />
        <div class="flex justify-end">
            <div class="float-right">
                <PrimaryButton class="bg-green-600 hover:bg-green-700"
                    title="agregar paciente que permanece hospitalizado." @click.prevent="agregarLineaEntrega">
                    +
                </PrimaryButton>
            </div>
        </div>
        <div class="flex flex-col pt-2">
            <div class="grid grid-cols-6 gap-4 p-2">
                ...
                <div><Label class="font-black" for="problemas" value="Problemas / intervenciones" /></div>
                <div><Label class="font-black" for="examenes" value="Examenes o Procedimientos Pendientres" /></div>
            </div>
        </div>
        <div class="flex flex-col">
            <div v-for="(entregado, index) in props.form.entregados" :key="entregado.id">
                ...
                <div class="grid grid-cols-6 gap-4 p-2">
                    <div class="flex flex-col md:flex-row">
                        ...
                        <div v-else class="flex flex-col md:flex-row md:items-center">
                            <div class="w-full md:w-3/4">
                                ...
                                <Input v-model="entregado.run" placeholder="Run ej: xxxxxxxx-x"
                                    :aria-label="`props.form.entregado.${index}.run`"
                                    @input="props.formatearRut(entregado.run, props.form.entregados, index, 'entregados')" type="text"
                            ...
                            <div class="mt-2 md:mt-0 md:w-1/4 md:ml-2">
                                <PrimaryButton v-if="validate(entregado.run)" class="bg-cyan-500 hover:bg-cyan-700"
                    ...
                    <div class="flex flex-col">
                        ...
                        <TextArea v-model="entregado.problemas" rows="3" cols="3"
                    ...
                    <div class="flex flex-col">
                        ...
                        <TextArea v-model="entregado.examenes" rows="3" cols="3"
