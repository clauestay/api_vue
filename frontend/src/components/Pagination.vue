...
defineProps({
    ...
    <nav v-if="data.links.length > 3" role="navigation" aria-label="Pagination Navigation"
        ...
        <div class="flex justify-between flex-1 sm:hidden">
            ...
            <Link v-else :href="data.prev_page_url" preserve-scroll
                class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 rounded-md hover:text-gray-500 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150">
            Previous
            </Link>
            <Link v-if="data.current_page < data.last_page" :href="data.next_page_url" preserve-scroll
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            ...
            <div>
                <span class="relative z-0 inline-flex shadow-sm rounded-md">
                    ...
                    <Link v-else :href="data.prev_page_url" rel="prev" preserve-scroll
                    ...
                    <template v-for="(link, key) in data.links">
                        <template v-if="key > 0 && key < data.last_page + 1">
                            ...
                            <Link v-else :key="`link-${key}`" :href="link.url" v-html="link.label" preserve-scroll
                                class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 hover:text-gray-500 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150"
                                aria-label="`Go to page ${link.label}`" />
                        </template>
                    </template>
                    <Link v-if="data.current_page < data.last_page" :href="data.next_page_url" rel="next" preserve-scroll
