                    ...
                    <template v-for="(link, key) in props.data.links">
                        <template v-if="key > 0 && key < props.data.last_page + 1">
                            ...
                            <span v-else-if="link.active" :key="`current-${key}`" aria-current="page">
                                <span
                                    class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-300 cursor-default leading-5">{{ link.label }}</span>
                            </span>
                            <router-link v-else :key="`link-${key}`" :to="link.url" v-html="link.label"
